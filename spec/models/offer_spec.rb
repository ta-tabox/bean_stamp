...
RSpec.describe Offer, type: :model do
  ...
  describe '#status' do
    ...
    context 'when an offer is on offer' do
      it { expect(offer.status[:class]).to eq 'on_offering' }
      it { expect(offer.status[:value]).to eq 'オファー中' }
    end
    context 'when an offer is on roasting' do
      it { expect(roasting_offer.status[:class]).to eq 'on_roasting' }
      it { expect(roasting_offer.status[:value]).to eq 'ロースト中' }
    end
    context 'when an offer is on preparing' do
      it { expect(preparing_offer.status[:class]).to eq 'on_preparing' }
      it { expect(preparing_offer.status[:value]).to eq '準備中' }
    end
    context 'when an offer is on selling' do
      it { expect(selling_offer.status[:class]).to eq 'on_selling' }
      it { expect(selling_offer.status[:value]).to eq '受け取り期間' }
    end
    context 'when an offer is end of sales' do
      it { expect(sold_offer.status[:class]).to eq 'end_of_sales' }
      it { expect(sold_offer.status[:value]).to eq '受け取り終了' }
    end
  end
end
